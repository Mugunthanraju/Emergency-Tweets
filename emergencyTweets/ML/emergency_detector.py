...
import ML.nlp_model as nlp_model
import pickle as pk
import ML.sarcasm_detector as sarcasm_detector
import ML.watson_nlu as watson_nlu
...
def is_eligible(tweets):
    ...
    """
    fear_matrix = [watson_nlu.is_fearful(
        tweet.text, nlu_api) for tweet in tweets]
...
def emergent_tweets(tweets):
    ...
    """
    try:
        eligible_tweets = [tweet for i, tweet in enumerate(
            tweets) if is_eligible(tweets)[i]]
